...
io.on("connection", (socket) => {
  ...
  socket.on("start", ({ roomId }) => {
    ...
    io.to(roomId).emit("changeTurn", { roomTurn: "gameStart", skipped: false }); //change the room's turn to startGame
  });
  ...
  socket.on("turnChange", ({ roomId, skipped }) => {
    ...
    if (skipped) {
      ...
      room.turn = newTurn;
      io.to(roomId).emit("changeTurn", { roomTurn: room.turn, skipped: false }); //change to the next turn
    } else {
